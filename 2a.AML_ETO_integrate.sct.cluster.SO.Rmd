...
```{r, "plotmissingintegrated",echo=FALSE}
if (TRUE) {
  for (myregress in names(expts.integrated.ls)) {
    ...
    counts.per.feat <- apply(myiso@assays$SCT.Diff@counts, 1, sum)
    cells.per.feat <- Matrix::rowSums(myiso@assays$SCT.Diff@counts > 0)
    
    # Plot counts and cells 
    plot(x = log2(counts.per.feat), y = log2(cells.per.feat), pch = 20, cex = 0.5,
         xlim = c(0, 25), ylim = c(2, 14))
    points(x = log2(counts.per.feat[lostFeat]), y = log2(cells.per.feat[lostFeat]), pch = 20, cex = 0.5, col = "red")
    
    plot(x = log2(counts.per.feat[lostFeat]), y = log2(cells.per.feat[lostFeat]), pch = 20, cex = 0.5, col = "red",
         xlim = c(0, 25), ylim = c(2, 14))
    points(x = log2(counts.per.feat[!(names(counts.per.feat) %in% lostFeat)]),
         y = log2(cells.per.feat)[!(names(cells.per.feat) %in% lostFeat)], pch = 20, cex = 0.5)
    
    for (myexpt in runs2process) {
