...
/home/groups/MaxsonLab/input-data/scRNAseq/200821_A01058_0073_AHNMNLDRXX
...
sourceDir = "/Users/lusardi/Documents/CEDAR/Projects/3.ASXL_mutant/1.AML_ETO/analysis/data/raw"
runs = list.dirs(path = sourceDir, full.names = FALSE, recursive = FALSE)

runs <- runs[c(4,6,2,7,5,8,3,9)]
runs
```
...
summary_ls = list()
for (myrun in runs) {
  myfiles = list.files(path = sprintf("%s/%s/outs", sourceDir, myrun))
  if (metricsFile %in% myfiles) {
    ...
    mydt = fread(sprintf("%s/%s/outs/%s", sourceDir, myrun, metricsFile))
    mycols = make.names(colnames(mydt))
    myclasses = metrics_colClasses[mycols]
    
    # Read in data 
    summary_ls[[myrun]] = read.csv(sprintf("%s/%s/outs/%s", sourceDir, myrun, metricsFile), colClasses = myclasses)
...
library(knitr)
print(summary.dt[, mget(cols2plot)])
axis_names = gsub("_", "\n", summary.dt$RunID)
xlimits = c(0, length(axis_names) + 1)
ylimits = c(1, max(unlist(summary.dt[, mget(c("Estimated.Number.of.Cells", names(cols2point)))])))
...
myplot = barplot(height = summary.dt$Estimated.Number.of.Cells, ylab = "Estimated Number of Cells", xlim = xlimits,
                 main = "Summary RNA seq by run", ylim = ylimits, log = "y", las = 1,
                 sub = "Number of Reads", cex.axis = 0.8)
text(myplot[,1], rep(0.25, length(axis_names)), labels = axis_names, xpd = TRUE, cex = 0.6)
myreads = sprintf(summary.dt$Number.of.Reads, fmt='%.2e')
text(myplot[,1], rep(0.05, length(axis_names)), labels = myreads, xpd = TRUE, cex = 0.7)
...
plotcolors = c(SCL200723TB_1_DMSO_mRNA = "purple", 
               SCL200723TB_2_AVA_mRNA = "red", 
               SCL200723TB_3_ORY_mRNA = "blue", 
               SCL200723TB_4_AVAORY_mRNA	 = "green") 
...
### Summary Antibody seq stats (ADT)
...
# Add some additional points per run
cols2point = list(Antibody..Number.of.Reads = list(color = "darkgreen", symbol = 17, legendtext = "# Ab Reads"),
...
par(mar=c(5.1, 4.1, 4.1, 9.1), xpd=TRUE)
myplot = barplot(height = summary.dt$Estimated.Number.of.Cells, ylab = "Estimated Number of Cells", xlim = xlimits,
                 main = "Summary Antibody seq by run", ylim = ylimits, log = "y", las = 1,
                 sub = "Number of Reads", cex.axis = 0.8)
text(myplot[,1], rep(0.25, length(axis_names)), labels = axis_names, xpd = TRUE, cex = 0.6)
myreads = sprintf(summary.dt$Number.of.Reads, fmt='%.2e')
text(myplot[,1], rep(0.05, length(axis_names)), labels = myreads, xpd = TRUE, cex = 0.7)
